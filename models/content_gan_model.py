...
from losses import init_loss, ContentLoss

class ContentGANModel(BaseModel):
	def name(self):
	...
	def initialize(self, opt):
		...
		if self.isTrain:
			...
			self.criterionGAN = networks.GANLoss(use_l1 = not use_sigmoid, tensor=self.Tensor)
			...
			self.contentFunc = self.contentLoss.contentFunc()
			self.criticUpdates = 5 if opt.gan_type == 'wgan-gp' else 1
			self.discLoss = init_loss(opt)s
			self.structLoss = torch.nn.MSELoss()
	...
	def backward_G(self):
		...
		# Second, G(A) = B
		self.loss_G_Content = self.contentLoss.get_loss(self.fake_B, self.real_B, self.contentFunc, self.structLoss) * self.opt.lambda_A
	...
	def get_current_errors(self):
		return OrderedDict([('G_GAN', self.loss_G_GAN.data[0]),
							('G_Content', self.loss_G_Content.data[0]),
