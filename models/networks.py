...
def define_D(input_nc, ndf, which_model_netD, n_layers_D=3, norm='batch', use_sigmoid=False, gpu_ids=[], use_parallel = True):
	netD = None
	use_gpu = len(gpu_ids) > 0
	norm_layer = get_norm_layer(norm_type=norm)
...
class ResnetBlock(nn.Module):
	...
	def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
		...
		if padding_type == 'reflect':
			...
			# block.add_module("0", nn.ReflectionPad2d(1))
			# blocks.append(nn.ReflectionPad2d(1))
		elif padding_type == 'replicate':
			conv_block += [nn.ReplicationPad2d(1)]
			# block.add_module("0", nn.ReplicationPad2d(1))
			# blocks.append(nn.ReplicationPad2d(1))
		...
		conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias),
					   norm_layer(dim),
					   nn.ReLU(True)]
		# blocks.append(nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias))
		# blocks.append(norm_layer(dim))
